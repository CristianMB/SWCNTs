...
%     B = 0;           %cm-1    
   ...
   %% GENERAL FUNCTIONS
    ...
    function normedSamples = Normalize(samplesToNormalize, lowerLimit, upperLimit, type)
    % Normalize the samples based on either the maximum value ('M') or integral value ('I') between the limits.
        ...
        for sampleIdx = 1:length(samplesToNormalize)
            ...
            % Check the type parameter to decide the normalization method
            if type == 'M'  % Normalization based on maximum value
                currentSample.Y = currentSample.Y / UsefulFunctions.ComputeMaximum(currentSample, lowerLimit, upperLimit);
            elseif type == 'I'  % Normalization based on integral value
                currentSample.Y = currentSample.Y / UsefulFunctions.ComputeIntegral(currentSample, lowerLimit, upperLimit);
            else
                error('Invalid type parameter. Use ''M'' for maximum normalization or ''I'' for integral normalization.');
            end
