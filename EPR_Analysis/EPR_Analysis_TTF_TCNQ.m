...
%             DATA_20250806.S060825A  %REF-AnnP2
            ...
            DATA_20250806.S060825L  %R13@2K
%             DATA_20250806.S060825M  %R13@5K
%             DATA_20250806.S060825N  %R13@10K
%             DATA_20250806.S060825O  %R13@20K
%             DATA_20250806.S060825P  %R13@55K
...
EPR_BG = CorrectEPRXAxis(EPR_BG,freq);
...
ExpTTF.Range=[330 345]; % mT
ExpTTF.Harmonic=1;

Sim_TTF.N = ['TTF Simulation g=' num2str(SysTTF.g)];
Sim_TTF.RS = 1;
...
ExpTCNQ.Range=[330 345]; % mT
ExpTCNQ.Harmonic=1;

Sim_TCNQ.N = ['TCNQ Simulation g=' num2str(SysTCNQ.g)];
Sim_TCNQ.RS = 1;
...
% EPR_All = [EPR_TCNQ(:); Simulations(:)];  % forces both to column vectors

% EPR_All = [EPR_TTF(:); Simulations(2)];  % forces both to column vectors
% EPR_All = Normalize(EPR_All,330,340, 'M');
% plotEPR(EPR_All, 1)
% 
% EPR_All = [EPR_TCNQ(:); Simulations(1)];  % forces both to column vectors
% EPR_All = Normalize(EPR_All,330,340, 'M');
% plotEPR(EPR_All, 1)
...
%     title('EPR Spectra', 'FontSize', 15, 'Units', 'normalized'); % Avoid overlap
    ...
    B_mT = linspace(minX, maxX, 10);   % You can increase number of ticks if needed    
